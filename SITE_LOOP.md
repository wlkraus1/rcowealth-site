...
- [ ] **R5. Life insurance prominence across the other 15 pages.** Carried over. Both homepages are
  ...
  - ✅ **`services.html` DONE 2026-08-08, iteration 18. Page 1 of 4.**
    **Measured first:** life insurance is **the fourth of four service cards**, and the hero lead names
    it last too. Its only CTA was the same "Open page →" every other card has, so **the services page
    had no route into the life funnel at all** — no calculator, no quoter.
    ⚠️ **The obvious fix was impossible and it is worth writing down why.** The cards are
    `<a class="card">` — **the whole card is the anchor**, so funnel links cannot be nested inside one
    without converting all four from `<a>` to `<div>` and rebuilding their click behaviour. **Reordering
    the cards to put life first was rejected as out of scope**: it is a positioning decision about how
    an RIA presents its service hierarchy, and that is Tyler's, not the loop's.
    ⚠️ **The homepage "protection band" could not be reused either.** `.protect-band` CSS lives in
    `revamp.css`, which **only `index.html` loads**, its markup now survives only in `mobile.html`, and
    the rules are orphaned on index. Copying a half-dead pattern into a third page would have spread it.
    **Built instead from components that already exist site-wide** — `.eyebrow.dark`, `.btn-primary`,
    `.btn-secondary.dark`, `.hero-actions` — so the callout needed no new component, only spacing.
    🚩 **A zero-gap defect was caught by measuring, not by looking, and that matters this iteration:**
    the callout initially butted **directly against the card grid, `gapPx:0` at both 1440 and 390**,
    which would have read as bolted on. **The browser pane had collapsed to `innerWidth:0`, so the
    screenshot came back blank and useless** — the geometry check found it anyway. Fixed with a 38px
    rhythm and a hairline `border-top`. **When the visual channel dies, measure the thing the eye
    would have caught rather than declaring it verified.**
    **Verified at 1440 and 390:** grid still has exactly 4 children with the callout outside it, gap
    38px, contrast **16.48 h3 / 5.11 body / 4.61 eyebrow**, buttons **45px**, no overflow, both links
    carrying `utm_campaign=website_services` in the site's existing convention.
    📝 **DRAFT FOR TYLER — this is the first new public copy the loop has written in this block.**
    Eyebrow *"Protection planning"*, heading *"Not sure how much cover your family would need?"*, body
    *"Start with the estimate, then compare real pricing from major carriers. The calculator asks for
    no contact information."* **Button labels are lifted verbatim from what is already live**
    ("Estimate your coverage need", "Compare real quotes"). No em-dashes, no performance language, no
    guarantee, no product named, and the no-contact-information claim matches what the calculator page
    already states. **Nothing is published; it is on the branch awaiting his approval.**
  - ✅ **`retirement-planning-greenville-sc.html` DONE 2026-08-08, iteration 19. Page 2 of 4.**
    **The page supplied its own hook, so the copy is not boilerplate.** Its review list already
    contains *"Survivor needs and legacy considerations"* — the callout expands the line the page
    already commits to, rather than importing the services pitch. **Deliberate, because R3 recorded
    that two thin near-identical pages is how the duplication problem this loop spent four iterations
    removing gets recreated.** Checked rather than assumed: **19% word overlap with the services
    callout, and the shared words are almost entirely the button labels** (`estimate`, `coverage`,
    `compare`, `real`, `quotes`), which *should* stay consistent. The prose is distinct.
    **The component ported with zero new CSS**, which is the payoff for having built it from
    site-wide classes on `services.html` instead of copying the orphaned `.protect-band`.
    **Verified at 1440 and 390:** callout outside `.page-grid`, gap **38px**, width matching the grid
    (1180 / 366), contrast **16.48 h3 / 5.11 body / 4.61 eyebrow**, buttons **45px** at 17.85 and
    16.48, no overflow, both links carrying `utm_campaign=website_retirement_planning`.
    ⚠️ **No screenshot again: the browser pane has been collapsed at `innerWidth:0` for two
    iterations**, so this was verified by geometry and computed contrast, not by eye. **Saying so
    rather than implying a visual check happened** — the gap defect on `services.html` proves the
    geometry check earns its place, but it is not the same as looking.
    📝 **DRAFT FOR TYLER:** eyebrow *"Survivor needs"*, heading *"What happens to the income plan if
    one of you is not there?"*, body *"Retirement income is usually planned around two people. If that
...
- **2026-08-08 — iteration 18, R5 page 1 of 4: `services.html`. A funnel entrance where there was none, and a defect the eye never got to see.** Measured first: life insurance was **the fourth of four service cards**, named last in the hero lead too, and its only CTA was the same "Open page →" as every other card. **The services page had no route into the life funnel at all** — no calculator, no quoter. ⚠️ **Two obvious approaches were dead ends and both are worth recording.** The cards are `<a class="card">`, so **the whole card is the anchor** and funnel links cannot be nested inside without converting all four to `<div>` and rebuilding their click behaviour. And the homepage protection band could not be reused: **`.protect-band` CSS lives in `revamp.css`, which only `index.html` loads**, its markup now survives only in `mobile.html`, and the rules are orphaned on index — copying a half-dead pattern into a third page would only spread it. **Reordering the cards to lead with life was rejected as out of scope**: how an RIA ranks its own services is a positioning decision and it is Tyler's. Built the callout from components that already exist site-wide instead, so it needed no new component, only spacing. 🚩 **The spacing is where this iteration earned its keep.** The callout first rendered **flush against the card grid, `gapPx:0` at both 1440 and 390** — it would have read as bolted on, and it is exactly the kind of thing you catch by looking. **The browser pane had collapsed to `innerWidth:0` and the screenshot came back blank**, so looking was not available. **Measuring the geometry the eye would have judged caught it anyway**, and it was fixed with a 38px rhythm and a hairline rule. **When the visual channel dies, measure the thing the eye would have caught — do not quietly downgrade to "verified".** Verified at both widths afterwards: grid still exactly 4 children with the callout outside it, contrast 16.48 / 5.11 / 4.61, buttons 45px, no overflow, both links carrying the site's existing `utm_campaign=website_services` convention. 📝 **This is the first new public copy the loop has written in this block, and it is a DRAFT for Tyler** — button labels lifted verbatim from what is already live, no em-dashes, no performance language, no guarantee, no product named, and the no-contact-information claim matches what the calculator page already says. **Nothing published; it sits on the branch for his approval.** **Three commercial pages left: `financial-advisor`, `investment-management`, `retirement-planning`.**
